...
describe("registerSchema", () => {
  ...
  describe("username validation", () => {
    ...
    it("should fail with username shorter than 3 characters", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((i) => i.message === "Username must be at least 3 characters"),
        ).toBe(true);
      }
    });
    ...
    it("should fail with username longer than 20 characters", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((i) => i.message === "Username must not exceed 20 characters"),
        ).toBe(true);
      }
    });
      ...
      if (!result.success) {
        expect(
          result.error.issues.some(
            (i) => i.message === "Username can only contain letters, numbers, and underscores",
          ),
        ).toBe(true);
      }
    });
  ...
  describe("password validation", () => {
    ...
    it("should fail with password longer than 16 characters", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((i) => i.message === "Password must not exceed 16 characters"),
        ).toBe(true);
      }
    });
    ...
    it("should fail with spaces in password", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((i) => i.message === "Password must not contain spaces"),
        ).toBe(true);
      }
    });
  });
  ...
  describe("confirmPassword validation", () => {
    ...
    it("should fail with empty confirmPassword", () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues.some((i) => i.message === "Please confirm your password")).toBe(
          true,
        );
      }
    });
