...
export class MediaStreamManager {
  ...
  private startStreamPromise: Promise<StartStreamResult> | null = null;
  ...
  async startStreamWithFallback(constraints?: MediaStreamConstraints): Promise<StartStreamResult> {
    ...
    if (this.startStreamPromise) {
      return this.startStreamPromise;
    }

    this.startStreamPromise = (async () => {
  ...
  async switchVideoDevice(deviceId: string): Promise<MediaStreamTrack | null> {
    ...
    // Notify listeners
    this.deviceSwitchCallbacks.forEach((callback) => {
      callback('video', newTrack.getSettings().deviceId || deviceId);
    });
  ...
  async switchAudioDevice(deviceId: string): Promise<MediaStreamTrack | null> {
    ...
    // Notify listeners
    this.deviceSwitchCallbacks.forEach((callback) => {
      callback('audio', newTrack.getSettings().deviceId || deviceId);
    });
