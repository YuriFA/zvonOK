...
while recording continues without interruption. The recording SHALL be
indicated in the UI while active (with elapsed time), SHALL start only from an
explicit user action, and SHALL stop and save when the user leaves the room or
closes the page. The control SHALL be hidden entirely when the browser
provides no usable media recorder, and disabled when no participant in the
room is publishing any media.
